...
export class SignalService {
    ...
    async encryptMessage(plaintext: string, remoteUserId: string, remoteDeviceId: number): Promise<any> {
        ...
        const boundCopy = new TextEncoder().encode(innerPayload).buffer;
        const boundCiphertext = await sessionCipher.encrypt(boundCopy);

        return {
            protocol: 'v3',
            type: boundCiphertext.type,
            body: this.arrayBufferToBase64(boundCiphertext.body as any),
            registrationId: boundCiphertext.registrationId,
            ...
            timestamp: Date.now(),
            messageType: boundCiphertext.type === 3 ? 'PREKEY' : 'WHISPER'
        };
    }
