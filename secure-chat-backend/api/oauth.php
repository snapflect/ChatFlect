...
try {
    ...
    // 0. Register Device (CRITICAL FIX for "Device Blocked" error)
    $deviceUuid = $data['device_uuid'] ?? 'unknown';

    // Dynamic Platform Detection
    $rawPlatform = 'UNKNOWN';
    if (isset($data['platform'])) {
        $rawPlatform = strtoupper(trim($data['platform']));
    } else {
    ...
    // Security Hardening: Strict Allowlist (Data Integrity)
    $allowedPlatforms = ['ANDROID', 'IOS', 'WEB'];
    $platform = in_array($rawPlatform, $allowedPlatforms) ? $rawPlatform : 'WEB';
    // Default to 'WEB' if unknown to prevent database pollution

    // Use ON DUPLICATE KEY UPDATE to ensure device is active upon login
    $devStmt = $conn->prepare("INSERT INTO user_devices (user_id, device_uuid, platform, last_active, status, created_at) 
                              VALUES (?, ?, ?, NOW(), 'active', NOW())
                              ON DUPLICATE KEY UPDATE last_active = NOW(), status = 'active', revoked_at = NULL, platform = VALUES(platform)");
    if ($devStmt) {
        $devStmt->bind_param("sss", $userId, $deviceUuid, $platform);
        $devStmt->execute();
        $devStmt->close();
    } else {
        error_log("OAUTH WARNING: Failed to prepare device registration statement: " . $conn->error);
    }
    ...
    // Log detailed error to file for forensics
    error_log("OAUTH CRASH: " . $e->getMessage() . " in " . $e->getFile() . ":" . $e->getLine());
    exit;
}
?>